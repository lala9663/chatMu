...
| 함수 | 상태 | 용도 |
|---|---|---|
| `spotify-connect` | 설계 | Spotify OAuth 콜백 — code를 token으로 교환, refresh token 저장 |
| `join-room` | 설계 | 방 코드 검증 후 room_members insert |
| `resolve-track` | 설계 | Odesli 조회 + tracks/track_links 캐싱 (rate limit 게이트) |
| `send-recommendation-push` | 설계 | 추천 수신자에게 Expo Push 발송 |
